...
def create_monopoly_board():
    board = [
        ...
        "║               ║     Ave       ║                ║               ║               ║",
        "╠═══════════════╬═══════════════╬═══════════════ ╬═══════════════╬═══════════════╣",
        "║    Jail       ║  Baltic Ave   ║     Chance     ║  Vermont Ave  ║ Connecticut   ║",
        "║               ║               ║                ║               ║     Ave       ║",
        "╠═══════════════╬═══════════════╬═══════════════ ╬═══════════════╬═══════════════╣",
        "║Free Parking   ║ Mediterranean ║ Community Chest║St. Charles    ║  Electric     ║",
        "║               ║     Ave       ║                ║     Place     ║   Company     ║",
        "╠═══════════════╬════════════════════════════════════════════════║════════════════",
        "║ Go to Jail    ║   Oriental    ║     Chance     ║ Electric      ║   St. James   ║",
        "║               ║      Ave      ║                ║    Company    ║     Place     ║",
        "╠═══════════════╬═══════════════╬═══════════════ ╬═══════════════╬═══════════════╣",
        "║ Pacific Ave   ║North Carolina ║ Community Chest║Pennsylvania   ║    Short      ║",
        "║               ║      Ave      ║                ║      Ave      ║     Line      ║",
        "╚═══════════════╩═══════════════╩═══════════════ ╩═══════════════╩═══════════════╝"
    ]
...
print_board(create_monopoly_board())
...
class Player:
    ...
    def _init_(self, name, initial_balance=1500, is_computer=False):
        self.name = name
    ...
    def prompt_sell_property(self, property):
        if self.properties:
            if self.is_computer:
                property_to_sell = self.properties[
                    0]  # Computer sells the first property in the list
                ...
                if sell_choice.lower() == 'y':
                    ...
                    property_to_sell = next(
                        (prop for prop in self.properties
                         if prop.name.lower() == property_to_sell.lower()),
                        None)
                    if property_to_sell:
                        self.sell_property(property_to_sell)
                    else:
...
def draw_chance_card(player):
    cards = [{
        "message": "You won a beauty contest! Collect 50 Kenyan Shillings.",
        "effect": lambda player: player.add_balance(50)
    }, {
        "message": "Pay hospital fees of 500 Kenyan Shillings.",
        "effect": lambda player: player.deduct_balance(500)
    }, {
...
def draw_community_chest_card(player):
    cards = [{
        "message": "Pay school fees of 800 Kenyan Shillings.",
        "effect": lambda player: player.deduct_balance(800)
    }, {
        "message": "Receive 100 Kenyan Shillings from sale of stock.",
        "effect": lambda player: player.add_balance(100)
    }, {
        "message": "Doctor's fee. Pay 500 Kenyan Shillings.",
        "effect": lambda player: player.deduct_balance(500)
    }, {
        "message": "Go to Jail. Pay 1000 Kenyan Shillings.",
        "effect": lambda player: player.deduct_balance(1000)
    }, {
        "message": "Income tax refund. Collect 50 Kenyan Shillings.",
        "effect": lambda player: player.add_balance(50)
    }, {
...
def take_turn(player, properties):
    ...
    if property_to_buy.owner and property_to_buy.owner != player:
    ...
    elif not property_to_buy.owner:
        ...
        print(
            f"{player.name} landed on their own property, {property_to_buy.name}."
        )
        ...
        if choice == '1':
            if not player.bankrupt and not computer.bankrupt:
                ...
                if game_continue:
                    ...
                    # Check winning condition
                    if computer.balance >= 2500:
                        print(
                            f"\nGame over! Computer has won by reaching 2500 Kenyan Shillings!"
                        )
                        game_continue = False
